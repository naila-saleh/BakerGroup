...
export default function Product(product) {
    return (
        <Grid key={product.id} size={{xl: 3, lg: 3, md: 6, sm: 6, xs: 12}}>
            <Link to={`/products/${product.id}`} style={{textDecoration: 'none'}}>
                <Card sx={{boxShadow: 3, borderRadius: 4, display: 'flex', flexDirection: 'column', justifyContent: 'center', gap: 1, cursor: 'pointer'}}>
                    <CardMedia component={'img'} image={product.mainImage} sx={{width: '100%', height: '260px', objectFit: 'cover', display: 'flex', alignSelf: 'center', borderRadius: 4}}></CardMedia>
                    <CardContent sx={{backgroundColor: '#2D5356', color: 'info.light', borderRadius: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: {xl: 2, lg: 1.3, xs: 2}, ":last-child": {paddingBottom: 2}}}>
                        <Box>
                            ...
                            {product.price==0?<Typography>custome price</Typography>:<Typography component={'span'} variant={'body1'}>₪{product.price}</Typography>}
                        </Box>
